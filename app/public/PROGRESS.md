...
- Committed and pushed as `459bf2b`, then deployed that exact SHA theme-only to the protected test site. WordPress and SiteGround caches were purged; the deployed data file, renderer and compiled CSS match the local SHA-256 checksums.
- Browser-checked all eight routes at 1440 x 900, 768 x 1024 and 390 x 844. Every route serves the dedicated H1, one active selector panel, five visible FAQs with FAQPage schema, a loaded hero and the correct desktop or mobile WindowCAD treatment. Maximum H1 size is 57.6px, no route has horizontal body overflow, click and arrow-key selector control both work, and there are no page-script errors. The only console noise is the existing WordPress jQuery Migrate `submit()` deprecation warning.
- All 24 unique product-image URLs referenced across the rendered routes return 200. Colour, privacy glass, handle and windows-hub destinations resolve successfully. Home, the windows hub, sliding sash, online quote, roof lanterns, flat rooflights, composite doors and heritage aluminium doors all return 200 after the deployment.
