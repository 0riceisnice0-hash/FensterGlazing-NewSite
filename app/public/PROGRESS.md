...
- Visually audited the scrape and built 77 responsive WebP assets from approved source imagery: the Grandeur hero, four collections, six portrait-first entrance views, eight colour examples shown on real doors, and six glass designs shown both on complete doors and as close details. Added a repeatable image build script and `COMPOSITE-DOOR-REDESIGN.md` maintenance guide.
- New journey: responsive Grandeur hero, corrected four-fact strip, Signature/Contemporary/nxt-gen/Grandeur selector, desktop comparison and mobile selected specs, portrait entrance gallery, mid-page quote prompt, interactive colour selector, interactive glass selector with door/detail modes, compact hardware section, FAQs, responsive quote, relevant review and compact Composite Doors enquiry.
...
- Refined the page after visual review: all route section headings are capped at `2.8rem` with a restrained 650 weight, gallery cards now respect the vertical shape of doors, selector previews keep a fixed 4:5 box, and Chatsworth and Wentworth use visible complete-door imagery rather than appearing blank. Colour and glass selections were interaction-tested, including the glass door/detail toggle.
- Built the compiled CSS/JS and linted all 61 theme PHP files. Deployed the refined theme through commit `3acd9ac` to the protected test site, flushed the WordPress and SiteGround caches, and verified the route at 390px, 768px and 1440px: no horizontal overflow or broken images; fixed-height selectors do not jump; headings resolve to 30.4px on mobile and 40.32px on desktop; the family carousel, desktop comparison, responsive quote handoff, footer trust grid and social links render correctly. Production was not deployed.
