...
- **Second follow-up:** owner said it was then too bunched up, and asked whether a read more would work on mobile or make a mess. It works here for a reason worth recording: **the two facts the intro turns on, that uPVC is a bonded foil and aluminium is powder coated, are both repeated in each material's own copy further down**, so collapsing it puts nothing behind a tap that is not also available in full below. The gap, overcorrected to 30px, went back to 49px, still well short of the 123px it started at.
...
- Verified at 390: intro 137px to 68px collapsed, copy block 404 to 372, 48% to 44% of the viewport, gap 49px, expand and collapse both flipping `aria-expanded` and the label. At 1440 the intro carries no class and the button stays hidden. **The saving is smaller than the intro shrank**, because the button itself costs height back.
