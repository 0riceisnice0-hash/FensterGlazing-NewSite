...
- Added an explicit `Before you chat` acknowledgement that keeps the composer unavailable until the visitor chooses `Continue to chat`. The disclosure explains that relevant page content and messages are processed by AI, warns that replies may be inaccurate and do not create quotations, contracts, warranties, professional advice or legally binding commitments, cautions against sensitive data and links to the expanded Privacy Policy. The choice lasts only for the current page visit and never changes a rejected or accepted optional-cookie preference.
...
- Rebuilt the theme, PHP-linted the component, REST integration and footer integration, deployed the theme to the protected test site, and verified the open/send/unconfigured-key flow at `1440 x 900`. The route accepted its nonce, returned the intended no-key state, removed the typing indicator, re-enabled the composer and produced no horizontal overflow or console errors. A live model response still requires the server key to be added and then tested.
