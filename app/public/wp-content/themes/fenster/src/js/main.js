...
/* CONSENT-FIRST. Nothing optional is on until the visitor presses a button.
   ...
   This replaces the 2026-08-09 granted-by-default model, under which an
   identifier was issued on the first paint, before the banner had rendered, and
   nothing ever took it back — so a visitor who then refused was still recorded
   as having consented for that page. */
...
/* Whether a real choice is stored, which under consent-first is a genuine
   question rather than always true. It distinguishes a refusal from an
   unanswered banner in the WindowCAD tracking value, so `rejected-cookies` means
   somebody said no and `cookie-consent-not-accepted` — unreachable under
...
const windowCadUrlWithReference = (value) => {
  ...
  try {
    ...
    /* Four outcomes and under consent-first ALL of them are reachable again,
       which they were not during the granted-by-default fortnight. In order of
...
/* THE QUOTE EMBED IS NOT GATED ON CONSENT, and that is a deliberate exception
   to consent-first rather than an oversight.
...
/* ---------------------------------------------------------------------------
   uPVC door randomiser (/upvc-doors/), 2026-08-12.

   Owner's idea, and deliberately not a configurator: it shuffles, it names what
   it landed on, and it hands over to WindowCAD for a price. Two configurators
   have been removed from this site on sight, and the difference this one relies
   on is that it never pretends to quote.

   HONESTY CONSTRAINT, and it is why the door image only changes with the
   finish: a real render exists for every finish, and none exists for this door
   wearing each handle or each glass, so those are presented as their own
   product images beside the door rather than composited onto it. Do not
   "improve" this by tinting the render or drawing glass into the panes.

   It never repeats the same finish twice in a row, because pressing a shuffle
   button and watching nothing change reads as a broken button.
   --------------------------------------------------------------------------- */
document.querySelectorAll('[data-fg-door-randomiser]').forEach((root) => {
  let payload;
  try {
    payload = JSON.parse(root.dataset.doorPayload || '{}');
  } catch (e) {
    return;
  }
  ...
  const colourChip = root.querySelector('[data-door-colour-chip]');
  const glassName = root.querySelector('[data-door-glass]');
  const glassNote = root.querySelector('[data-door-glass-note]');
  const glassChip = root.querySelector('[data-door-glass-chip]');
  const handleName = root.querySelector('[data-door-handle]');
  const handleImage = root.querySelector('[data-door-handle-image]');
  const live = root.querySelector('[data-door-live]');
  const button = root.querySelector('[data-door-shuffle]');
  if (!image || !button) return;

  // Preload every render once, so a shuffle swaps instantly rather than
  // flashing an empty frame on a slow connection.
  finishes.forEach((f) => {
    if (!f.image) return;
    const pre = new Image();
    pre.src = f.image;
  ...
  const apply = (finishIndex) => {
    const finish = finishes[finishIndex];
    const glass = glasses.length ? glasses[Math.floor(Math.random() * glasses.length)] : null;
    ...
    if (glass && glassName) {
      ...
      if (glassNote) glassNote.textContent = glass.privacy ? 'Privacy ' + glass.privacy + ' of 5' : '';
      if (glassChip) {
        glassChip.style.background = glass.image ? 'url("' + glass.image + '") center / cover' : (glass.texture || '#eef4f4');
      }
    }
    ...
    if (handle && handleName) {
      ...
      if (handleImage) {
        if (handle.image) {
          handleImage.src = handle.image;
          handleImage.alt = handle.name + ' long-plate handle';
          handleImage.style.display = '';
        } else {
          handleImage.style.display = 'none';
        }
      }
    }

    // The visual change is meaningless to a screen reader, so the combination
    // is announced instead.
    if (live) {
      live.textContent = finish.colour
