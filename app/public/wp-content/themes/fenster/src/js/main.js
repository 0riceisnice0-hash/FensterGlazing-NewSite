...
// Casement windows: accessible opening-style explorer.
document.querySelectorAll('[data-fg-casement-explorer]').forEach((explorer) => {
  const tabs = [...explorer.querySelectorAll('[data-fg-casement-tab]')];
  const panels = [...explorer.querySelectorAll('[data-fg-casement-panel]')];
  if (!tabs.length || tabs.length !== panels.length) return;

  const selectTab = (nextIndex, moveFocus = false) => {
    const index = Math.max(0, Math.min(tabs.length - 1, nextIndex));
  ...
  tabs.forEach((tab, index) => {
    ...
    tab.addEventListener('keydown', (event) => {
      ...
      event.preventDefault();
      selectTab((nextIndex + tabs.length) % tabs.length, true);
    });
  });
});
