...
<div class="fg-cw">
    ...
    <?php
    /* Read from the same source the colour options page uses, so the two cannot
       drift apart. Smooth white carries no image because it is the unfoiled
       profile, and falls back to a flat block the way the colour carousel does. */
    $casement_colours = fenster_data('colour_options.materials.upvc.colours', []);
    $casement_colours = is_array($casement_colours) ? $casement_colours : [];
    ?>
    <?php if (! empty($casement_colours)) : ?>
                    ...
                    <?php foreach ($casement_colours as $colour) : ?>
                        ...
                        <li>
                            <?php if (! empty($colour['image'])) : ?>
                                <img src="<?php echo esc_url(fenster_generated_url((string) $colour['image'])); ?>" alt="<?php echo esc_attr(sprintf('Liniar uPVC window profile in %s', $colour_name)); ?>" loading="lazy">
                            <?php else : ?>
