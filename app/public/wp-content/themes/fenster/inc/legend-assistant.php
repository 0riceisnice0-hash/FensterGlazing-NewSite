...
function fenster_legend_verified_business_context(): string
{
    ...
    $facts = [
        ...
        'Heritage windows and heritage aluminium doors are the Sheerline Classic system and Fenster fits the stepped sash on both, which is double glazed. Triple glazing on Classic needs the contemporary sash instead, so it is not part of the standard offer and is not shown on the product pages, but it can be done on both products on request. If a visitor asks about triple glazing on either, say it is possible with the contemporary sash and ask them to raise it with the team. Do not present it as a normal option and do not quote a U-value for it.',
