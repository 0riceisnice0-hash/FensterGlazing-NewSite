...
function fenster_google_ads_feed_is_authorized(WP_REST_Request $request): bool|WP_Error
{
    ...
    /*
     * Basic auth is only on offer when BOTH halves are configured, and this
     * has to fail closed. hash_equals('', '') is true, so an unset password
     * previously let the hardcoded default username plus an empty password
     * authorise the whole feed whenever a token was configured. A token-only
     * deployment must authenticate with the token.
     */
    $basic_auth_available = $expected_username !== '' && $expected_password !== '';
...
function fenster_google_ads_conversion_rows(): array
{
    $query = new WP_Query([
        ...
        'post_status' => 'private',
        /*
         * Newest first, so if the window ever holds more enquiries than the
         * cap it is the already-imported older rows that fall off the end
         * rather than the new conversions. Google dedupes on order_id, so
         * re-sending a row that was imported yesterday costs nothing.
         */
        'posts_per_page' => 2000,
        'fields' => 'ids',
        'orderby' => 'date',
