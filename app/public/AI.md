...
- **THE SECTION DRAWING CARRIES NO DIMENSIONS AND MUST NOT GAIN ANY.** Sheerline publishes no figure for how far a standard sash stands proud, so the step is labelled in words. It IS exaggerated for legibility — at true proportion the two profiles read as identical where the SVG renders — and that is honest only because the drawing is captioned as simplified and states no measurement. Add a number and it becomes a false claim.
...
- **WE HAVE NO PRESTIGE FLUSH CORNER RENDERS, and the ones we do have show the opposite product.** All twelve are `Classic-Corner-*`: Sheerline **Classic** is the other range and its corner has a **stepped profile with the sash standing proud**. They render on this page through `$aluminium_colour_routes`, directly below a section explaining that a flush sash does not do that. The grid's job is colour and its alt text says "frame corner", which is why it was left rather than pulled — but **ask Sheerline for Prestige flush corners**, and it is a one-line data swap when they arrive.
