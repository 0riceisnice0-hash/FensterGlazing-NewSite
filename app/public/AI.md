...
- **Live theme is at scoped release `616d673` on `codex/cookie-consent-live`; test theme is at `0c1d567` code / `5081b4d` docs on `main` as of 2026-07-31.** Both now have the polished granular modal and corrected Legend/native-dialog state handling. Production was built from exact Ads live base `814b8bf`, so its Ads attribution/feed work remains intact; test still contains the separate privacy-glass review batch. The trees therefore remain intentionally different and `main` is not safe to deploy wholesale. Verify by checksum rather than trusting this line.
