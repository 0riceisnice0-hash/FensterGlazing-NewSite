...
- **Handheld before-and-after pairs do not register, and the eye is a bad judge of it.** These were 30px right and 178px down at full resolution. Measure the offset off a patch of FRAME, never glass — the glass is the thing that changed — then verify at two different heights, which also tells you whether there is rotation as well as translation. A 50/50 blend is the check: the frame edges must resolve to single lines.
