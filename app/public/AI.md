...
- **Live theme is at `23d31c4` on `codex/cookie-consent-live` as of 2026-07-30.** This scoped release was built from the checksum-confirmed Ads production state and adds only the approved granular cookie consent. `main` remains ahead with the separate privacy-glass test batch, so it is not safe to deploy wholesale. Verify by checksum rather than trusting this line.
