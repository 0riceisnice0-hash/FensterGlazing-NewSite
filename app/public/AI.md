...
- **Live is at `ef2b6c3` as of 2026-07-22.** Live Google reviews and the rebuilt showcase are on production, along with the composite-doors work from the concurrent session, all shipped with explicit owner approval after range checks. Live and `main` are level. Verify the live theme rather than trusting this line.
