...
- **Live theme remains at `814b8bf` on `codex/ads-production-release`; test theme is at `0c1d567` on `main` as of 2026-07-31.** Production still has the earlier dismissible Accept/Reject banner. Test has the polished granular modal and fixes Legend's old banner-offset observer so a native open dialog hides Legend instead of moving it and leaving it displaced. `main` also contains the separate privacy-glass test batch, so it is not safe to deploy wholesale. Verify by checksum rather than trusting this line.
