...
- ~~**The case-study strip is gated off** because no study claims the route.~~ **Ungated 2026-08-11**, the same day it was added: the Heal's study claims `/heritage-windows/`, so the route shows one card that is genuinely its own. If that study is ever unpicked, put the gate back rather than letting `fenster_case_studies_for_product()` fall through to ALL studies. The two heritage DOOR studies still do not count: those jobs had no windows in them.
