...
- **Composite doors: the standard range plus any RAL.** Owner-confirmed, 2026-07-29. The swatches on `/composite-doors/` and `/colour-options/` are a **selection** of the standard range, not the whole of it, and beyond the standard range any RAL colour can be matched. Both pages must say so. This reverses the July removal of the blanket RAL claim, which came off as unsubstantiated at the time; `COMPOSITE-DOOR-REDESIGN.md` carries the same correction. **It does not extend to heritage aluminium doors**, which stay at twelve standard powder-coated finishes with dual and bespoke colours on request.
