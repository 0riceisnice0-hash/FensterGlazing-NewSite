...
- **`/handle-options/` is the handle hub for the whole site: windows, tilt and turn, and doors, with patio to come.** Owner instruction, 2026-07-29. Every family renders through one shared component, `template-parts\components\handle-chooser.php`, so they are presented identically and there is only one copy of that markup.
