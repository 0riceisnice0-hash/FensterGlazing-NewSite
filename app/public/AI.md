...
- Cookie consent is versioned and granular in `fenster_cookie_consent`: `analytics` controls Clarity plus `FG2`/`FGV` website journeys, and `marketing` controls Meta, advertising tags and persisted ad click IDs. **Both optional categories are granted by default and stay on until the visitor refuses** — owner instruction, 2026-08-09, superseding the 2026-07-30 default-off model. The tools load on the first page view and every visitor gets an `FG2` journey and a stamped WindowCAD URL from the first paint. The first-visit modal still appears and still records `banner_shown`, but it no longer blocks and no longer holds tracking back; dismissing it leaves the defaults in place, so footer **Cookie settings** is always visible rather than appearing only after a choice. The Customise switches are preselected from the effective state, because showing them off while the tools are demonstrably running would be a lie. A default is never written to storage — only a real choice is — so the banner keeps appearing until one is made. A saved choice lasts 180 days. Old `accepted`/`rejected` string values are deliberately invalid so the granular version is collected afresh.
...
- **This default is weaker than the ICO's position** that consent must precede non-essential storage and that default-on is not valid consent. It was raised before implementing and it is the owner's decision. Do not quietly re-tighten it, and do not describe it in customer-facing copy as consent having been obtained first — banner, privacy policy and cookie policy copy all state that the categories are already on and how to turn them off, and they must be kept true if the default changes again.
- Only analytics consent may create `FG2`/`FGV`, page/click/time events, WindowCAD joins or dashboard journey events. Marketing consent is separate and may not silently create an analytics identity. A WindowCAD quote without analytics consent must use the separate WindowCAD **Tracking** field value `rejected-cookies`. **Since 2026-08-09 that value means a real refusal**, because analytics is granted by default; `cookie-consent-not-accepted` is now unreachable and its branch survives only so flipping the default back restores it, and because historical leads still carry it. Both still go to the office, but neither may be relayed into the dashboard as individual events. A WindowCAD completion without a consented `FG2` value is relayed only into the aggregate-only statistical path (`quote_completed`, device class `server`) so total completions stay measurable without creating a journey.
...
- **It is enforced in `fenster_case_studies_for_product()` and `fenster_case_studies_for_product_group()`, not in per-route gates.** Every caller of both is a residential page: the shared product journey, casement, roof lanterns, heritage doors and the three product hubs. Commercial routes render their own templates and do not call either. One rule, one place. Both take a type argument if a commercial page ever needs the same behaviour.
...
- **WHEN THE CLIENT AND THE BUILDING ARE THE SAME ORGANISATION, NAME IT.** Owner ruling, 2026-08-10, on the Roehampton study: "it's a building, not a contractor." The rule above separates the property from the firm that engaged us, and that works when a main contractor sits in between, as at Bletchley. It collapses when an institution commissions work on its own premises. The reasoning behind the original rule is the test: the poaching risk is a **contractor or developer who buys glazing repeatedly** and can be lifted as a warm lead. An institution procuring its own estate through tender is not that.
