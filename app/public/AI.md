...
- **Live is at `94e7d0f` as of 2026-07-24.** Live and `main` are level, and test is at the same commit. The last release was the product-imagery split that gave six window/door routes their own gallery pools. Verify the live theme rather than trusting this line: it was four commits stale before that deploy, and the only files that distinguished the two candidate commits were `inc/generated-pages.php` and `assets/email/google-reviews.png`.
