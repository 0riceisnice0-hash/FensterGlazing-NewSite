...
- **Scope is `/patio-doors/` only**, the uPVC sliding route. Mila's literature says the ProLinea suits PVCu, timber and aluminium patio doors, but Sheerline's slider has not been confirmed as taking it, so `/aluminium-sliding-doors/` renders no handle section rather than an assumed one. Confirm with the owner before widening the slug list.
