...
- **Live is at `984e89c` as of 2026-07-27.** Live, `main` and test are all level. The last release was the product-selector hub rebuild: `/windows-milton-keynes/`, `/doors-milton-keynes/` and `/other-services/` on one `product-hub.php` (the old `windows-hub.php` is gone), a one-viewport tile grid, a decision panel with FAQs, and the Notan mark. Verify the live theme by checksum rather than trusting this line.
