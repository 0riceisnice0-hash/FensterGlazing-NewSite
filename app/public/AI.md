...
- **A route having a page, copy and imagery does not mean it is on its hub.** `/cat-and-dog-flaps/` had all three and was still missing from `other-services` until 2026-07-29, so the only ways in were a direct link or search. The homepage product theatre had described Other Services as covering cat and dog flaps the whole time. When adding or reviving a route, check `product_hub_groups` as well as the route, the media and the nav.
