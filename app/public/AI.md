...
- **Narrow exception — Legend QA.** The test-only Legend assistant may send its actual user/assistant transcript to the authenticated Marketing Dashboard after the separate chat acknowledgement. It is retained for 30 days, disclosed in the chat notice and Privacy Policy, and must never be put in `website_events`, AdminBase or general analytics. With optional-cookie acceptance it is linked to `FGV`/`FG2`; after rejection it is chat-only, with no `FGV`/`FG2`, journey or website-event record.
...
- Keep the explicit pre-chat acknowledgement in front of the composer. It should open with the short agreement that using live chat permits AI processing and asks visitors not to share sensitive information; a `Read chat terms` disclosure must expose AI processing, possible inaccuracy, non-binding replies, the 24-hour same-browser history, 30-day QA retention and the Privacy Policy before the visitor starts. After acknowledgement, `fenster_legend_chat_v1` may hold the acknowledgement and up to 16 recent messages for continuity across Fenster pages and tabs. Keep the TTL, Clear chat control and disclosure. This must never accept, overwrite or otherwise change the visitor's separate optional-cookie choice.
