...
## Casement Designer Rule

- The designer on `/casement-windows/` lives in `template-parts\components\casement-designer.php` with its controller under `[data-fg-casement-designer]` in `src\js\main.js`. It draws the window the visitor has specified: six layouts, seventeen exterior faces, three glazing-bar treatments, horns, six handle finishes, two interior faces, obscure glass and a continuous size.
- **It earns its place on the same test the blind visualiser passed.** That combination is tens of thousands of images, so no photograph set can answer it. See the Three.js / Canvas Rule: this is 2D canvas with no library, and it is 2D because face on a hinged sash projects to an exact quadrilateral. Do not "upgrade" it.
- **The open state is calculated, not drawn by eye.** A sash hinged on one edge and rotated by `t` projects its face to `w*cos(t)` and the far side of the slab to `w*cos(t) + DEPTH*sin(t)`; at 90 degrees that leaves exactly one thickness, which is the sash seen edge on. `OPEN_ANGLE` is 58 degrees because at 34 the sash only lost a sixth of its width and the state read as a sash that had shrunk. Do not reduce it without checking the drawing still reads as open.
- **Every customer-visible value comes from `inc\site-data.php`.** Foils from `colour_options.materials.upvc.colours`, handle finishes from `window_handles.finishes`, woodgrain from the `grain` flag on the three oak and rosewood foils. No hex, no finish name and no colour count belongs in the component or the controller. This is the Swatch Provenance Rule, and `grain` follows the same pattern as `glitter` on the Notan slats.
- **Nothing hit-tests the canvas, and that is deliberate.** Every control is a real `button` or a native `input type="range"`. The blind visualiser had to grow padded grab elements because a `pointerdown` handler cannot cancel a scroll the browser has already committed to; with no canvas gesture at all that failure cannot happen here and the stage stays scrollable on touch. Do not add a canvas drag.
- **Three things the drawing teaches that the prose could not**, and they are the reason it exists rather than being decoration. A fixed pane is glazed straight into the frame, so it visibly holds more glass than an opener the same size. Georgian bars sit inside the sealed unit and the reflection passes over them, while astragal bars sit on the face and cast a shadow. And an open sash shows a smooth white rebate unless the foil is specified on both faces, which is what `colour_options` already says in words. Keep all three.
- **A control that changes no pixel reads as broken.** Handles are on the inside face and Match colour inside is only visible from indoors, so both turn the view round when pressed. Check any new control against this before adding it.
- **The drawing is proportional and prints no face width.** No frame, sash or sightline dimension appears on the page, for the same reason no slat width appears on `/integral-blinds/`: we do not have those figures published. The size sliders describe the customer's own opening, and the panel says where a single sash versus a mullion is settled rather than inventing a maximum Liniar do not publish.
- `is-live` is added only after a first frame is painted. Until then the Cranfield photograph shows, so a thrown error degrades to a real casement window rather than to an empty box.
- Verify it by hashing the canvas before and after every control rather than by looking at it. That is what caught Match colour inside changing nothing.
