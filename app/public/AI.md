...
- **FENSA does not apply on `/secondary-glazing/`, `/roofline/` or `/integral-blinds/`, and step 04 is swapped on all three.** Owner-confirmed 2026-08-07. A FENSA certificate covers replacement windows and doors: secondary glazing is an additional window inside the existing one, roofline is fascia, soffit and guttering with no glazing in it, and integral blinds are not a replacement window either. All three were promising a document that never arrives.
- **They are NOT given three hand-written journeys.** They take the canonical steps and replace one sentence, from `order_process.aftercare_without_fensa` in `inc/site-data.php`. Three near-identical aftercare paragraphs is how the six-step-sets mess of 2026-07-29 starts again.
- **The guarantee sentence in that replacement is word for word the canonical one.** It is already scoped to "new windows and doors" precisely so it stays true on these routes, which sit outside the ten year insurance-backed cover. Do not shorten it, and do not "fix" it to claim the cover for these products.
- Commercial and pet flaps still pass genuinely different journeys of their own. These three do not: their journey is the same, one clause of it was untrue.
