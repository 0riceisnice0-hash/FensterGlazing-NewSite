...
- **There is no per-finish photography and it is not an oversight.** VBH render levers in only three of the five finishes we sell, none at all in White, and show brushed as a finger cup rather than a lever, so no complete set exists. The finishes therefore render as colour chips and the section carries the real lever-and-cup pair instead. **Do not tint a chip or generate a tile to fill the gap**, which is the rule already recorded for the four composite colours with no painted tile. If VBH ever supply a full labelled set, add `image` keys to the finishes and both components pick them up with no template change.
- `handle-grid.php` and `handle-chooser.php` both degrade when a finish has no image: the grid draws a chip from `hex`, and the chooser falls back to a single `fallback_image` rather than emitting an empty `src`. Keep both fallbacks; an empty `src` is a broken image and makes the browser re-request the page.
