...
- **Live is at `4458fc6` as of 2026-07-24.** Live, `main` and test are all level. The last release carried the enquiry-type audience default, the heritage configuration cut, the colour hub hero, the homepage card edge and the roof lantern configuration wall. Verify the live theme by checksum rather than trusting this line; on 2026-07-24 it was found four commits stale.
...
- Before changing product imagery, open the images. Filenames and imported alt text lie: `Casement-Windows-Flitwick-10.jpg` is a bay window and was the `/casement-windows/` hero, and the `/french-casement-windows/` hero was a Sheerline aluminium window on a uPVC page. Check the rendered page as well as the data, because a photograph whose subject sits in a corner will be cropped out of a body or gallery cell.
...
- **The range is one grid of tiles that fits a single desktop viewport, and it must stay that way.** No filters, no category bands, no per-card spec boxes, no separate configurations section: with nine items there is nothing to narrow down, and every control added pushes the range further off screen. Owner instruction, 2026-07-24, after all four were tried and rejected as over-complication.
...
- The windows panel opens by correcting the assumption that aluminium is the warmer material. It is not: our uPVC reaches 0.95 W/m²K against 1.0 for aluminium. Keep that, and keep the guarantee FAQ that names what the ten years does **not** cover.
