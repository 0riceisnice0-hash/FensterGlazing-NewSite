...
- **`/secondary-glazing/` carries its own four steps**, owner-confirmed 2026-08-07. The canonical step 04 promises a FENSA certificate and FENSA does not apply to secondary glazing, because nothing is being replaced. This is the third route allowed its own `steps`, after commercial and pet flaps, and like both of those it is owner-confirmed rather than inferred. Steps 01 and 03 are the canonical wording unchanged, so the rail still reads as one process.
...
- **FENSA is not relevant to secondary glazing, and the rail no longer says it is.** Owner-confirmed 2026-08-07: "fensa is not relevant to secondary". A FENSA certificate covers replacement windows and doors; this product is an additional window fitted inside the one already there, so nothing is replaced and no certificate follows. The route now passes its own four steps from `generated-page.php`. Steps 01 and 03 are the canonical wording untouched because both are true here; 02 drops thresholds for reveal depth, and 04 drops the certificate. **04 says ten years and deliberately does not repeat "insurance-backed through the CPA"**, which is scoped to new windows and doors everywhere else and has not been confirmed for this product. Ask before widening it.
