...
- **Repair prices come from the office Customer Repairs Price List and nowhere else.** `OneDrive/Office Information/Price Lists/Repairs Price List/Customer Price List 04022025.pdf`, whose own header reads "Last updated: 12/02/25". They live in `repair_problems` and `repair_prices` in `inc/site-data.php`; nothing is priced in a template.
- **The page publishes eight of the eighteen rows, plus the minimum charge.** That is a deliberate selection, not the list. Do not paste the rest in: the eight are the faults people actually ring about, and a full trade price list on a customer page invites line-by-line argument at the door.
- **Figures on the page are inc VAT** because that is the number a homeowner is buying. The list's "multiples" column is exc-VAT throughout — it says "£90 each + VAT" and "£50 + VAT" outright, and its bare numbers match the exc column — so those are converted: £30 → £36, £2.00 → £2.40, £80 → £96, £50 → £60, £90 → £108. **That reading is an inference, not something the list states.** If the office means them as inc-VAT, those five are the numbers to change.
- **These are public and indexable, so their accuracy is a live-content responsibility**, the same standing the price guides have. The source list is dated February 2025; re-check it against the office before assuming a figure is current.
- **Do not claim a guarantee on repair work.** The ten year insurance-backed CPA cover is on new windows and doors and repairs sit outside it. The key-specification strip on that route claimed "Guarantee: 10 years" until 2026-08-06 while the process rail on the same page correctly said "new windows and doors"; the page contradicted itself for months. Nothing on a repair route may state or imply that cover.
