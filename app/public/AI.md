...
- **Live theme was rolled back to `814b8bf` on `codex/ads-production-release` on 2026-07-30.** Production again has the earlier dismissible Accept/Reject cookie banner. The granular consent work remains on test/`main`; `main` also contains the separate privacy-glass test batch, so it is not safe to deploy wholesale. Verify by checksum rather than trusting this line.
