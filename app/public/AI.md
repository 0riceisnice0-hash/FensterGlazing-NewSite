...
- Live is at `de13375` as of 2026-07-20 (About redesign, price guide redesign, Nick's review-links/nav work, the six-fixes batch: composite colours hub, case-study strips, related-link image cards, footer socials, obscured-glass picker rebuild), with live and test byte-identical and nothing staged on test. New agents should verify the live theme rather than trusting this line.
...
- The WindowCAD **Tracking** field is external configuration that can silently break: on 2026-07-15/16 the WindowCAD website designer form settings were edited (the "Where did you hear of us?" dropdown work) and the Tracking property was dropped from the retail/door form field lists, so the site's `tracking=FG2-…` URL parameter had nothing to fill and every lead lost its tracking number. The Tracking property must be present in the WindowCAD website designer form fields, not merely defined on the account. Theme defences in `inc/adminbase.php`/`inc/website-tracking.php`: any submitted field carrying a valid `FG2-` value is accepted, a submission with no tracking value at all logs a warning and adds a visible "Website tracking: none" line to the AdminBase notes, and the aggregate `quote_completed` relay makes the breakage visible on the dashboard within a day.
