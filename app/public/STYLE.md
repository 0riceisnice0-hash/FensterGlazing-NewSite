...
- **EVERY SECTION OF A PAGE FITS ONE DESKTOP VIEWPORT.** Owner instruction, 2026-09-03, on /sliding-sash-windows/: *"every section of this page needs to be contained in one view port. every section."* Given for that page and applied there (see PROGRESS, same date); treat it as the standard for any page he reviews next. Under the header that is 827px at 1440x900 and 791px at 1536x864. The things that break it are always the same: section padding over 3rem, display headings that wrap to three or four lines, a form that stacks short fields, a copy panel that sets the height of the component beside it. Measure with a script over every top-level section, not by scrolling and guessing.
...
- **Steps are labelled, not numbered.** "Style" and "Finish", not "01 Style" and "02 Finish" (owner, 2026-09-03). Numbering turns a choice into a form. See Important Updates.
- **A configurator section fits one desktop viewport.** Head, stage and controls under the header on a 1440x900 screen with room to spare, and the stage height follows the viewport with a floor so a shorter laptop screen still gets the whole thing. The sliding sash lock selector is the reference: 717px tall at 1440 wide after the owner asked for it "a bit shorter" because it "just about" did not fit.
