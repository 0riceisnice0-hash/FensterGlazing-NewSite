...
- **Heritage windows and heritage aluminium doors are double glazed, and triple is an on-request conversation rather than an option on the page** (owner, 2026-08-11). Both are the Sheerline Classic system and Fenster fits the stepped sash, which does not take triple; the contemporary sash does, and it is deliberately not offered as standard so the Classic range stays distinct from Prestige. Legend may confirm triple is possible on either product with the contemporary sash and ask the visitor to raise it with the team. It must not present it as a normal option or quote a U-value for it, and the product pages deliberately say nothing about it.
