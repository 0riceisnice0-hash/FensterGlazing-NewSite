...
2. ~~**GTM only exists after Accept.**~~ **Partly closed 2026-08-02.** `inc/consent.php` now sets Consent Mode v2 defaults (`ad_storage`, `ad_user_data`, `ad_personalization`, `analytics_storage` all denied) before anything loads, and sends an `update` on every choice. What has *not* changed is that gtm.js still only loads after a choice, so reject and no-choice visitors send no denied-state pings and remain unmodelled. Sizing the undercount from the dashboard's consent counters is still the honest reading. Loading GTM before a choice so the denied pings can fire remains an owner decision about a deliberately strict consent layer.
3. ~~**No gclid capture.**~~ **Closed.** `gclid`, `gbraid` and `wbraid` are captured, held against the consented journey and written to `fenster_enquiry` post meta (`_fenster_ad_click_id`, `_fenster_ad_click_type`), alongside an `ads={adgroupid}` tracker. Marketing consent gates all of it and none of it reaches the Marketing Dashboard. Manual UTMs on the account tracking template are still worth adding (`utm_source=google&utm_medium=cpc&utm_campaign={campaignid}`), because Google auto-tagging uses gclid and the tracker's first-touch context reads UTMs, so without them the dashboard still files ad clicks as organic Google.
