...
- **Live is at `984e89c` as of 2026-07-27, and live, `main` and test are all level.** The last release rebuilt the three product-selector hubs (`/windows-milton-keynes/`, `/doors-milton-keynes/`, `/other-services/`) onto one `template-parts/sections/product-hub.php`, driven by `product_hub_groups` in `inc/site-data.php`, replacing the retired `windows-hub.php`. It is a one-viewport grid of product tiles with a system mark, a decision panel that compares systems on real figures without ranking them, and four FAQs per hub. `/other-services/` is now a real page rather than a scrape shell. Backup: `fenster-pre-984e89c-20260727-083131.tar.gz`.
...
- Consent health is aggregate-only (`accepted`, `rejected`, `acceptance rate`). Rejected visitors must never get a visitor/journey ID or browsing event. Banner impressions are intentionally not counted: anonymous crawler/session traffic made them misleading. Future Focus Group call integration should send actual call outcomes into the dashboard only after an API/webhook or scheduled export is available; phone taps alone remain intent, not confirmed calls.
...
- Cookie settings is deliberately a footer control, not a persistent viewport button. It reopens the existing consent banner, which is the only cookie UI Legend observes for vertical clearance. Do not reintroduce the floating `.fg-cookie-settings` control.
...
- The generic product-page redesign remains deployed on live for routes that have not received a dedicated template. The eight older window routes have a newer shared dedicated renderer on test: `casement-windows`, `flush-casement-windows`, `french-casement-windows`, `tilt-turn-windows`, `bow-bay-windows`, `aluminium-windows`, `aluminium-flush-windows` and `heritage-windows`. Their data is in `inc\window-product-data.php`, their renderer is `template-parts\sections\window-product.php`, and the router dispatches them before the generic product journey. They share a page framework but each route owns its facts, option selector, explanations, FAQs and form source. Liniar backs the five uPVC routes; Sheerline Prestige or Classic backs the aluminium routes. Manufacturer exports are research sources only; all runtime imagery is copied and optimised into `assets\images\products`. Do not restore the generic order rail, supplier filler or imported mini-gallery on these eight pages.
