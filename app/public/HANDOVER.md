...
- **Live theme remains at `814b8bf` on `codex/ads-production-release`; test theme is at `0c1d567` on `main` as of 2026-07-31.** Production still has the earlier dismissible Accept/Reject banner. Test has the polished granular modal and the corrected Legend/native-dialog state handling, alongside the separate privacy-glass test batch, so do not deploy `main` wholesale. Verify by checksum rather than trusting this line.
