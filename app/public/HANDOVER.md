...
- Mobile colour/glass choices are a compact horizontal rail. The furniture section is the fixed-height Globe/Acorn selector (2026-07-16) at every width, with all thirteen finish images loaded eagerly so the stage never empties between picks. **Since 2026-09-03 (test `5d68383c`) the thirteen are the colour guide's own photographs at 478x316 (`<finish>-478w.webp`), not the 232px crops, and the Globe and Acorn style buttons each carry a 3:2 tile of that furniture on a rail** (`globe-on-rail-*.webp` from Roseview's render, `acorn-on-rail-*.webp` from the 1536px studio photograph in the old site export). No public source has the locks larger than this; originals would have to come from Roseview.
