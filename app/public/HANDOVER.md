...
- **Specification choices is route-specific here.** Frame colour is not a decision on this route: the unit goes into whichever window or door is being made and takes that frame's colour. The first card points at the slat colours on the hub instead. The privacy glass card stays, because that choice is real. The colour card's dots are frame finishes hardcoded in the stylesheet, so this one passes its own six through from the data; without that it advertised green and orange slats.
