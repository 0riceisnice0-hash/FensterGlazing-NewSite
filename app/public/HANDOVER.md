...
- 2026-07-21 tracking audit: WindowCAD leads stopped carrying tracking numbers after 2026-07-14 because the WindowCAD account's website designer form settings were edited (adding/making required "Where did you hear of us?") and the custom **Tracking** property (`isHiddenInOffice`) was dropped from the retail/door form field lists. The theme, live URL rewriting, REST callback and dashboard join were all verified working. Fixing it requires re-adding the Tracking property to the WindowCAD website designer form fields (owner/office action in WindowCAD settings). The theme now scans all submitted fields for an `FG2-` value, warns in logs and AdminBase notes when a submission has no tracking value, and relays unattributed completions into the dashboard's aggregate-only `quote_completed` statistic so the Website Tracker shows an amber alert when this happens again. Separately, no WindowCAD callbacks at all reached WordPress between 2026-07-16 09:56 and the audit; the endpoint tested healthy, so confirm with the office whether WindowCAD itself received quotes in that window.
