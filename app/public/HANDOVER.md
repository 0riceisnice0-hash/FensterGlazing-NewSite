...
`codex/seo-landing-pages-2026-09` and deployed to protected test at `2bc795e0`,
following owner approval. Read `SEO-PERFORMANCE-AUDIT-2026-09-14.md` and the newest
`PROGRESS.md` entry for scope and server verification. Production is unchanged.
Do not read the old test/main table below as this workspace's current revision.
...
from the theme directory on protected test after deployment. Real WordPress
validation now passes for 573 area pages and seven price guides. Sixteen HTTP
page checks pass, as do sampled browser checks for real reviews, mobile menus,
FAQs and layouts. No actual enquiry or third-party quote was submitted.
...
- **NEVER cut a release from a superseded tag.** There are five, and only the
  ...
  live-seo-2026-08-31f             f26681b0   superseded
  live-casestudy-2026-09-03        0b4a1385   superseded
  live-casestudy-2026-09-03b       3aa1e851   superseded
  live-casestudy-2026-09-04        b389183d   LIVE
  ```

- **Either build on `b389183d`, or ship a range from `main`** once the rest of
...
- Dedicated Roseview comparison content lives in `template-parts\sections\generated-page.php` for Ultimate Rose, Heritage Rose and Charisma Rose. **Since 2026-09-02 it is the `.fg-sash-stage` component, ON TEST as `39c618f7` and not yet live**: a pale showroom on the page canvas in which one model stands large in the middle on a lit floor with its reflection, the other two small and angled in at the sides, brought forward by clicking a side window, the arrows, a stop on the meeting rail scale (which is the switch, with a gliding marker) or a swipe, with that model's facts beside it on white as hairline rows, ending in two paragraphs on what those figures mean (`meaning` on the model data). The selected window carries a white name tag under it at every width. The facts cross-fade rather than re-animate. The eight differences live once in `$sash_comparison_rows`. Test is `39c618f7`. A dark version of the same stage was on test for one deploy on 2026-09-02 and was pulled on the owner's instruction; see the no-dark-backgrounds rule in `STYLE.md`. See the Sliding Sash Roseview Rule in `AI.md`.
...
- Privacy glass on this route is the shared `privacy-glass-card` component with sash wording (since 2026-09-03, test `55ea3233`), not the generic specification-choices band, which is gated off for the sash slug. The furniture section is the lock selector `.fg-sash-kit` (rebuilt 2026-09-03): a pale stage with the guide photograph blended onto it by multiply, a caption bar, two picture tiles for Globe and Acorn and finish chips with metal discs. All thirteen finish photographs (`<finish>-478w.webp`, the colour guide's own images at 478x316) load eagerly so the stage never empties between picks; the tiles are `globe-on-rail-*.webp` (Roseview's render) and `acorn-on-rail-*.webp` (the 1536px studio photograph from the old site export). No public source has the locks larger than this; originals would have to come from Roseview, and any replacement set must be on pure white for the blend.
- Every section of the sash page fits one desktop viewport at 1440x900, 1536x864 and 1920x1080 (owner rule, 2026-09-03; test `857124bd`). The stage is sized to be exactly the viewport under the header and its facts panel is out of flow so it cannot push past that; since `e8b7b36e` it is one full-width surface (the section paints a pale room that fades to transparent at both ends, the scene and the facts have no boxes, the frame is up to 1640px wide, and since `e460ec02` the windows are on the left with one detail column on the right (name, one line on what it suits, the rail photograph and seven fact rows; the explanatory paragraphs are in the data but not rendered), no intro paragraph, no meeting-rail scale and no survey note, the two arrows overlaying the scene (`42c3077b`)); the gallery and the CTA band under it are transparent over the page canvas (they were a dark steel band and a solid pale band, and each drew a hard line against the canvas); the case-study strip and enquiry section have sash-page variants (tighter padding, two-column short fields) in the `SLIDING SASH: ONE VIEWPORT PER SECTION` block of `main.scss`.
- The sash page order is hero, key specs, the Roseview stage, the thermal camera comparison (`fg-sash-thermal`, added 2026-09-04 from the owner's Claude Design file; figures computed in PHP from the published U-values), Real homes, the furniture selector, privacy glass, the instant quote, the reviews, the case studies, the FAQs, the enquiry form (owner, 2026-09-03; test `f7dd41e1`). The FAQs are captured with an output buffer in `generated-page.php` and echoed after the case studies, on this slug only, because they are built inside the product-journey block and the sections they follow are outside it. The "Your project" quote band was deleted on the same instruction. Measure this page at 1440x780, not 1440x900: that is nearer the window the owner reviews on.
