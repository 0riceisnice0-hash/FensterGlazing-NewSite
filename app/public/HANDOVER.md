...
- Dedicated Roseview comparison content lives in `template-parts\sections\generated-page.php` for Ultimate Rose, Heritage Rose and Charisma Rose. **Since 2026-09-02 it is the `.fg-sash-stage` component, ON TEST as `4cf04368` and not yet live**: a dark steel showroom band in which one model stands large in the middle on a lit floor with its reflection, the other two small and angled in at the sides, brought forward by clicking a side window, the arrows, a stop on the meeting rail scale (which is the switch, with a gliding marker) or a swipe, with that model's facts beside it on dark glass as hairline rows, ending in two paragraphs on what those figures mean (`meaning` on the model data). The selected window carries a white name tag under it at every width. The facts cross-fade rather than re-animate. The eight differences live once in `$sash_comparison_rows`. Test is `4cf04368`. See the Sliding Sash Roseview Rule in `AI.md`.
