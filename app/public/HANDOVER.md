...
- **The key-specification strip prints one starred figure on the Liniar routes**, listed in `single_u_value_routes` in `inc\site-data.php`, and both glazing figures live on the EnergyPlus banner. Owner instruction 2026-08-04. Add the Sheerline routes to that list when that call is made.
- **The locking mechanism is deliberately unbranded.** Kenrick Excalibur was offered as "pretty sure" and is nowhere in this project; do not print it until it is confirmed.
