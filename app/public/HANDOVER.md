...
- When a visitor asks a factual Fenster question, Legend's backend searches a bounded local index of other published theme/WordPress pages and supplies up to four relevant excerpts if matches exist. This is same-site retrieval, not open-web browsing. The chat renderer supports `**bold**` through safe DOM text/`strong` nodes only; all other model output remains inert text.
- Legend must remain Fenster-only. The instruction block uses a fixed short redirect for unrelated requests, while server-side conversation and response filtering redacts common profanity before it can reach or be repeated by the model. Do not weaken this boundary when expanding chat features.
- Legend's composer is protected by an explicit per-page acknowledgement covering AI processing, possible inaccuracies, non-binding replies and sensitive-data caution, with a direct Privacy Policy link. This is deliberately separate from analytics/marketing-cookie consent: using the chat never changes `fenster_cookie_consent`, and a rejected choice must remain rejected.
