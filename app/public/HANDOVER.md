...
- **Live theme is at scoped release `616d673` on `codex/cookie-consent-live`; test theme is at `0c1d567` code / `5081b4d` docs on `main` as of 2026-07-31.** Both have the polished granular modal and corrected Legend/native-dialog state handling. Production was rebuilt from exact Ads live base `814b8bf`, preserving its attribution/feed work; test still carries the separate privacy-glass review batch. Do not deploy `main` wholesale. Verify by checksum rather than trusting this line.
