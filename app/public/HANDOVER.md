...
- The test contact page now uses the shared form's reusable `consultation_booking` mode: it presents the next 30 days of weekdays, 9am-4pm preferred-time requests, then saves and emails the selected date/time as a consultation request. It is a request flow, not live availability; the office must confirm the appointment.
