...
- The shared form's reusable `consultation_booking` mode now has a dedicated indexable `/book-a-consultation/` page as well as the Contact-page section. The dedicated page follows the quote-tool conversion model: hero form, trust bar, booking steps, review proof, visible FAQs/FAQPage schema and related internal links. The desktop header and Contact consultation card now lead to this canonical route. It saves and emails the selected date/time as a consultation request. It is a request flow, not live availability; the office must confirm the appointment.
