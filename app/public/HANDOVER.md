...
- The shared form's reusable `consultation_booking` mode has a dedicated indexable `/book-a-consultation/` page as well as the Contact-page section. The dedicated page follows the quote-tool conversion model: hero form, the same four-card reviews/accreditations proof row used below the homepage hero, booking steps, an advice/contact section, visible FAQs/FAQPage schema, review proof and related internal links. Its date stage is a compact six-week calendar card sized to the interaction, with a concise availability strip: Monday-Friday, 9am-4pm, excluding England-and-Wales bank holidays. The final details stage is a dedicated light-surface form with an appointment summary, legible bordered fields and a clearly contained consent/submit area rather than the shared dark-form styling. The official GOV.UK holiday feed is cached and enforced in both the picker and submission validation. Its background is one continuous `--fg-page-gradient` canvas, per `STYLE.md`. The desktop header and Contact consultation card now lead to this canonical route. It saves and emails the selected date/time as a consultation request. It is a request flow, not live availability; the office must confirm the appointment.
- The consultation hero uses locally hosted real Fenster imagery: the Milton Keynes showroom and a completed home installation. This is the accepted model for adding meaningful local proof without turning the booking panel into a decorative gallery. The `Prefer to speak now?` panel uses subtle icon-led phone/email actions. A 2026-07-14 wholesale rebuild of this page into a generic image-card composition was rejected and reverted; do not replace this page's approved structure without explicit owner approval.
