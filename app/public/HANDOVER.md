...
- The test contact page now uses the shared form's reusable `consultation_booking` mode: one fixed-size panel replaces the date calendar with 9am-4pm preferred-time choices, then the customer's details, with back controls at each step. The desktop header has a matching dark-blue `Book consultation` action that takes customers straight there. It saves and emails the selected date/time as a consultation request. It is a request flow, not live availability; the office must confirm the appointment.
