...
- **`main` now contains the live line: `codex/tracking-repair` was merged in on 2026-08-02.** Live itself has not moved and is still the `289b2c2` runtime (tip `f37e05d`). `main` is a superset of it, so a release from `main` no longer deletes the Ads attribution, the conversion feed or the consent metrics. **Still re-establish live by checksum before deploying rather than trusting this line**, which has been stale four releases running.
