...
- **Live is at `01dba14` as of 2026-07-28, and live, `main` and test are all level.** That release shipped two sessions' work at once. This session's half: consultations stated as free sitewide, the Commercial mega menu (Services and Sectors columns plus CTAs, driven by `mega`/`columns`/`ctas` in `inc/site-data.php`), six commercial sector pages and an AOV page in `inc/commercial-product-data.php`, commercial case studies moved to their own `/commercial-projects/` route with 301s from the old `/case-studies/` URLs, and a sitewide tag restyle onto `--fg-tag-*` tokens. The other half was the casement windows page rebuild and the named-technology banner. No pre-deploy tarball; rollback point is `984e89c`, checksum-verified against live before the deploy.
