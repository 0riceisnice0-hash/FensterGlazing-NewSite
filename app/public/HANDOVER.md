...
- Local pending work on 2026-07-07 adds a dedicated commercial product renderer for `/commercial-windows-and-doors/`, `/curtain-walling/`, `/louvre-vents/`, `/commercial-automation/` and `/healthcare-construction/`. It uses `inc\commercial-product-data.php` plus `template-parts\sections\commercial-product.php` and bypasses the generic generated product journey for those routes. At the time of this note it is local-only and still needs test deploy/approval before live.
