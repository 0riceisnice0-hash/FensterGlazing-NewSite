...
- **Live is at `5b7a612` as of 2026-07-29, and live, `main` and test are all level.** Recent releases: `01dba14` shipped two sessions' work at once (casement rebuild, commercial mega menu, six sector pages, `/commercial-projects/`, sitewide tag restyle); `2f78837` reworked the footer trust tiles and social links; `5b7a612` rewrote `/commercial-glazing-leeds/` off its scraped listicle, corrected the casement U-value to 0.95 W/m2K on the 36mm triple, and added the uPVC colour grid to seven product routes. **The colour grid is a shared component**, `template-parts/components/upvc-colour-grid.php`, driven by `colour_options.materials.upvc.colours` and gated by `$upvc_foil_routes` in `generated-page.php`. Sliding sash (Roseview) and secondary glazing (aluminium) are excluded on purpose.
