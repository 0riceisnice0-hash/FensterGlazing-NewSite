...
- Every section of the sash page fits one desktop viewport at 1440x900, 1536x864 and 1920x1080 (owner rule, 2026-09-03; test `857124bd`). The stage is sized to be exactly the viewport under the header and its facts panel is out of flow so it cannot push past that; the gallery is a light band now, not dark steel; the case-study strip and enquiry section have sash-page variants (tighter padding, two-column short fields) in the `SLIDING SASH: ONE VIEWPORT PER SECTION` block of `main.scss`.
