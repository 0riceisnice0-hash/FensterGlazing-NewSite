...
- The wall list is deliberately rendered twice. The second pass is `aria-hidden` with class `is-clone` and exists only so the desktop drift loops seamlessly. At `860px` and below, and under `prefers-reduced-motion`, the drift never starts, the clone is hidden and the viewport is a plain native scroll rail. Keep all three of those behaviours together; an auto-running marquee fights touch scrolling.
- **The drift is JavaScript, not a CSS animation** (`[data-fg-door-wall]` in `src/js/main.js`). It is driven by `scrollLeft` so a drag and the automatic movement act on the same property instead of fighting. The wall can be grabbed and panned, wheeled, or tabbed to and arrow-keyed. It pauses on hover, while dragging and for `1400ms` after a drag, then resumes once the pointer leaves.
- **Do not increment `scrollLeft` directly.** Reading it back rounds a sub-pixel step away and the wall simply never moves. The offset is held in a variable, written to the element each frame, and resynced from `scrollLeft` whenever the visitor scrolls it themselves.
- QA note: headless Chrome reports `prefers-reduced-motion: reduce` by default, so the drift looks broken in automated checks unless you override the media feature with `Emulation.setEmulatedMedia`.
- `.fg-cd3-colour` is the colour wall: paint chips left, live preview right. Hovering, focusing or tapping a colour previews it. The preview resolves in three steps: a door **we** photographed (`colours/`), then a Distinction render of a door in that named colour (`colour-doors/`), then the brush-stroke paint sample (`palette/`). Seventeen of the twenty seven colours currently reach a door. The caption says which the visitor is looking at.
- **Only map a colour to a door when the source filename names that colour.** Never infer a colour by eye from a render, and never tint an image to fake one. Nine colour doors were added this way from files such as `GD12-Purple-Violet.jpg` and `CL01-70-Colza-Yellow.jpg`; see `COLOUR_DOORS` in the build script.
- Keep the paint chips at a **4:3 aspect**. They were once cropped to thin strips and the brush stroke stopped reading, which the owner rejected. Never replace them with flat CSS colour blocks; the texture is the point. The blanket "any RAL colour" claim stays excluded per `AI.md`.
- Hover is an enhancement layered on top of click, so touch devices are never stranded. If you add interaction here, keep click working.
- **The tabbed colour/glass/hardware configurator was removed on 2026-07-22** at the owner's request, and with it the decorative glass designs and handle finishes. Those no longer appear anywhere on this route. Restore from git history if the owner wants them back, and give them their own section rather than reviving the tabs.
...
- **Both card rows are a fixed height**, set by `--fg-card-caption` and the image panel's `aspect-ratio`. The caption box is identical on every card whatever the copy runs to, so nothing is content-driven and the longest paragraph cannot set the height of the row. The paragraph is `-webkit-line-clamp: 4` purely as a guard; at the current copy lengths nothing actually clips, so no card shows an ellipsis. If a description is rewritten longer, check it still fits before shipping rather than raising the clamp.
- **The caption overlaps the base of the image panel** by `--fg-card-overlap`, which the figure reserves as bottom padding. That is what makes the overlap safe: the caption rides over the tint, never over the door. Do not remove the padding to "tighten" the panel, and do not increase the overlap past the reserved padding, or the door gets cut off again.
