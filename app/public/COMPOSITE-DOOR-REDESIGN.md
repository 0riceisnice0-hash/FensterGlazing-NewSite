...
- `.fg-cd3-colour` is the colour wall: swatches left, live preview right. Hovering, focusing or tapping a colour previews it. **The nine colours we have photographed on a real door come first and show the door; the remaining Distinction paint colours show the brush-stroke paint sample instead.** Never tint an image to fake a colour we have not photographed, and never replace the brush-stroke swatches with flat CSS colour blocks; the texture is the point. The caption states which of the two the visitor is looking at. The blanket "any RAL colour" claim stays excluded per `AI.md`.
