...
- **Live is at `834b424` on `main`, deployed 2026-07-29. Test is ahead at `4d72959` and awaiting approval.** Live and `main` carry identical theme code at `834b424`; test carries the colour rail work on top of it. `main` sits a commit or two ahead because the commit that records a release is written after it; those are documentation only and never change the theme, which is exactly why the deployed SHA and the newest SHA differ and why the checksum check matters. Backup before it: `~/backups/fenster-theme/fenster-pre-834b424-20260729-161416.tar.gz` (371M). The range `d3600ad..834b424` was **54 commits from two sessions**: 52 product, imagery, copy and navigation changes, and 2 from the concurrent session adding Google Ads conversion tracking (gclid capture and a form-submit dataLayer event). The owner was told the batch contained both before approving it, and the Ads code was checked against the consent boundary before shipping: the click id is not persisted for a rejected visitor and the dataLayer push is gated on `trackingConsentAccepted()`.
...
- The previous release was `4458fc6` (2026-07-24). Before that, `94e7d0f`.
