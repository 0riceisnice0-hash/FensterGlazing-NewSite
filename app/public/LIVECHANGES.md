...
- **`main` is ahead of live by seven unshipped commits** (`ecb5689`..`5f4b6b9`: casement gallery + consent work, pushed by another session on 3 Aug). They are not test-verified for release by this deploy. The next release from `main` ships them — verify them on test first or cherry-pick around them.
