...
- **Live is `0d283b4` on `release/flush-and-glass` as of 2026-08-06, established by checksum on `assets/css/main.css` and the casement template against `56015ba` immediately before the rsync, and re-verified on three files after it.** **Live is deliberately NOT the tip of `main`.** `main` also carries an eighteen-commit online-quote strand from another session that the owner has not signed off, so this release was cut from live and the twenty-four flush/glass commits cherry-picked onto it. Content: the flush casement page rebuilt as a bespoke middle, and the obscured glass hub (hero wall, house-first scene, real drag on touch, texture corrections). Backup: `fenster-pre-flushglass-20260806-122356.tar.gz` (393M), proven in the deploy script by extracting the casement template out of the tarball and hashing it. Socket purge `msg:OK`. Verified as a visitor: twelve routes `200`, both pages carrying their new markup, and `/online-quote/` still `200` because that route predates the other session's work and falls back to the shared template without it.
