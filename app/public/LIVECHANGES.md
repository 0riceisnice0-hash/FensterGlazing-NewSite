...
- **Live is at `41ffb83` on `main`, deployed 2026-07-29. Test is ahead at `8fb4fe2` (colour rail snap) and awaiting approval.** Backup before it: `~/backups/fenster-theme/fenster-pre-41ffb83-20260729-183206.tar.gz` (375M, 1,738 entries, confirmed to exist before deploying). Live was re-established by checksum first and the recorded pointer was accurate again: three theme files matched `8940ef6..4cd1925`, all theme-identical, so `8940ef6` was used as the range base to yield a superset. The range `8940ef6..41ffb83` was **19 commits, all one author**: the handle options box on the colour hub, the mobile hero rebuild, the read more on the three colour collections, and the composite white tile reframe. Verified on production: four theme files byte-identical, six routes 200, the handle box linking to `/handle-options/` with five finishes, three read more controls with matching ids, no duplicate eyebrows, 336 hero tiles, and all three ranges in the owner's order. Behaviour checked at both widths rather than assumed: at a true 390 all three collection paragraphs collapse to 51px with their controls showing and the hero background carrying the fade; at 1440 none are clamped, all three controls stay hidden and the hero background is `none`.
...
- The previous release was `6ea0dba` (2026-07-29). Before that, `834b424`, `4458fc6` (2026-07-24) and `94e7d0f`.
