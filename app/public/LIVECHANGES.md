...
- **Test is ahead at theme commit `7d6c84c` with a new granular cookie-consent modal, awaiting approval.** A first visit now opens a native modal that cannot be dismissed until the visitor chooses Customise or Accept all. Use necessary only remains inside Customise, where analytics and marketing are separate and off by default. Valid choices last 180 days, and footer Cookie settings supports withdrawal. Analytics gates Clarity and `FGV`/`FG2`; marketing gates Meta, advertising tags and persisted ad click IDs. Google Tag Manager receives category-specific Consent Mode signals before it loads. Legacy accepted/rejected strings are intentionally re-prompted. Live has not been changed.

## Current Truth

- Active GitHub repo: `https://github.com/0riceisnice0-hash/FensterGlazing-NewSite`
- **Live is at `572fe3c` on `main`, deployed 2026-07-29. Test is ahead at `4c794c8` (privacy glass card: full-width desktop, non-overlapping mobile strip with a top fade, Satin swatch corrected) and awaiting approval.** Backup before it: `~/backups/fenster-theme/fenster-pre-572fe3c-20260729-190129.tar.gz` (375M, 1,738 entries, confirmed to exist before deploying). Live re-established by checksum first and the pointer was accurate for the third release running: two theme files matched `22df227..ef7c419`, all theme-identical, so `22df227` was the range base to yield a superset. The range `22df227..572fe3c` was **4 commits, all one author**, only one of which touches the theme: the colour rails moved from mandatory to proximity snap and the drag release stopped forcing the nearest slide. Verified on production: two theme files byte-identical, five routes 200, `.fg-colour-rail__track` serving `scroll-snap-type:x proximity`, the other eleven rails still on mandatory, and both drag cases driven against the live page with `scrollTo` intercepted, a 24px nudge releasing to 94px and a 240px fling to 960px rather than being pulled to 851.
