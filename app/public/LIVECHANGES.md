...
- **Live is at `8052f65` on `main`, deployed 2026-07-29.** Live, `main` and test are all level. Backup before it: `~/backups/fenster-theme/fenster-pre-8052f65-20260729-083544.tar.gz` (371M). The range `5b7a612..8052f65` was 3 commits: the S2 Signature handle finishes laid out on the five window routes that already qualified for the handle card, and a small hover lift on the swatch and handle tiles. Earlier the same day: `5b7a612` (Leeds rewrite, casement technical data, uPVC colour grid) and `2f78837` (footer).
