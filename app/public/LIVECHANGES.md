...
- **Live is at `94e7d0f` on `main`, deployed 2026-07-24.** Live and `main` are level, and test is at the same commit. Backup before it: `~/backups/fenster-theme/fenster-pre-94e7d0f-20260724-085656.tar.gz` (361M). The release was the product-imagery split for six window/door routes; the range `fa6596b..94e7d0f` was two commits, both this session's.
- **The live pointer in this file was stale by four commits before that deploy** (it still claimed `b0ec36a` and the `release/heritage-doors` branch). Production was actually `fa6596b`, established by checksumming `inc/generated-pages.php` and `assets/email/google-reviews.png` against history — those two files were the only discriminator between `5672fb9` and `fa6596b`. **Always re-establish live by checksum rather than trusting this line**, and correct it as part of the deploy.
