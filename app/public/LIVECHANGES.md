...
- **Live is at `b0ec36a` on the `release/heritage-doors` branch, deployed 2026-07-21.** That commit is `658ba34` (the previous live state) plus the dedicated `/heritage-aluminium-doors/` page and nothing else. Backup before it: `~/backups/fenster-theme/fenster-pre-b0ec36a-20260721-161757.tar.gz`.
- **Live is deliberately NOT `main`, and the two have diverged.** `main` carries another session's composite-door redesign (`ac0f573`, `81a932c`, `dc0f240` and the earlier `e0be345..d106c20` run) and two new case studies (`b48893a`, `b07c12d`) that the owner has not approved for production. `git log 658ba34..ce4c76c` was nineteen commits of which only seven were the approved heritage work, so the release was rebuilt on top of the live commit instead of deploying `main`. Before the next live deploy, work out which of those composite/case-study commits the owner actually wants and build the release the same way.
- Test currently mirrors live plus nothing: it was last deployed at `b0ec36a`. The heritage work also exists on `main` as `c0376a7..ce4c76c`; the release branch is a re-application of it, not a second implementation.
