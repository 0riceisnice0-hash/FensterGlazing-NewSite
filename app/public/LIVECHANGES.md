...
- **`main` and the live line are merged again as of 2026-08-02.** `codex/tracking-repair` (runtime `289b2c2`, tip `f37e05d`) is merged into `main`, so the 71/23 commit split recorded here is closed. Nothing is deployed from that merge yet: **live is still running `289b2c2` and must be re-established by checksum before the next release**, not read from this line.
