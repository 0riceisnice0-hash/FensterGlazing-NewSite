...
- **Live theme is at scoped release `616d673` on `codex/cookie-consent-live`; test theme is at `0c1d567` code / `5081b4d` docs on `main`, retaining the separate privacy-glass batch for review.** The live release was rebuilt from exact Ads production base `814b8bf`; only the six consent/Legend source, policy and compiled files differ from that base, while the Ads attribution/feed files remain byte-identical. A fresh 375M/1,739-entry backup was extracted and compared with live before promotion. Production checksums match the candidate, six routes return 200, the first layer exposes only Customise and Accept all, optional scripts stay absent for necessary-only and load for accept-all, Legend hides at offset `0px` while the modal is open and its 420px panel opens normally afterward. Desktop and 390x844 checks show no horizontal overflow or console errors. Test was restored to `main` after the promotion and remains intentionally different from live.
