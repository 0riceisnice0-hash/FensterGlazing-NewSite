...
- **Live is at `4458fc6` on `main`, deployed 2026-07-24.** Live, `main` and test are all level. Backup before it: `~/backups/fenster-theme/fenster-pre-4458fc6-20260724-102814.tar.gz` (364M). The range `94e7d0f..4458fc6` was six commits, all one session's: the enquiry-type audience default, the heritage configuration cut, the colour hub hero, the homepage card edge and the roof lantern configuration wall.
- The previous release was `94e7d0f` (product imagery for six window/door routes), backup `fenster-pre-94e7d0f-20260724-085656.tar.gz`.
