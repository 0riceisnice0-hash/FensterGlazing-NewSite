...
- **Live is at `01dba14` on `main`, deployed 2026-07-28.** Live, `main` and test are all level. The range `984e89c..01dba14` was 92 commits from two concurrent sessions: the casement windows page rebuild and the named-technology banner across the Liniar and Sheerline routes (47 commits), and the free-consultation pass, the Commercial mega menu, six commercial sector pages, the `/commercial-projects/` route with its curated case studies, and the sitewide tag restyle (45 commits). **No pre-deploy tarball was taken.** Rollback is `git reset --hard 984e89c` in the server repo followed by the same theme-only rsync; `984e89c` was checksum-verified against live immediately before the deploy, so it is an exact recovery point. Last tarball on the server remains `fenster-pre-984e89c-20260727-083131.tar.gz`.
