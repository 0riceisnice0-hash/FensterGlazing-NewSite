...
- **The polished granular cookie-consent modal is live through scoped release `616d673`.** First visits require Customise or Accept all, with necessary-only inside Customise and separate analytics/marketing choices. Legend hides while the native modal is open and returns at its normal `0px` offset when it closes; the old banner-only height observer no longer leaves Legend displaced.

## Current Truth

- Active GitHub repo: `https://github.com/0riceisnice0-hash/FensterGlazing-NewSite`
- **STOP BEFORE ANY LIVE DEPLOY. Live is not running `main`.** Measured 2026-08-02 by checksum, not read from this file: `inc/consent.php`, `assets/css/main.css`, `assets/js/main.js` and `inc/case-studies-data.php` on live all match **`origin/codex/tracking-repair`** (head `f37e05d`, 31 July) byte for byte. That branch is **not merged into `main`**, and this line previously claimed `616d673` on `codex/cookie-consent-live`, which is wrong: tracking-repair was released after it. **Fourth release running where this pointer was stale, so keep checksumming rather than trusting it.** The two lines have diverged in both directions: **71 commits on `main` are not live** (the colour hub, heritage doors, casement and Wolverton case study work), and **23 commits are live but not on `main`**, including `inc/google-ads-conversions.php`, which is 317 lines that do not exist on `main` at all, roughly 400 lines of `website-tracking.php`, the consent metrics, and the clearing of `_gcl_`, `_gac_`, `_gads` and `_gpi` when a visitor rejects consent. **Deploying `main` as it stands would delete all of that, including a live privacy behaviour.** Both sides also modify the same files: `heritage-aluminium-doors.php`, `casement-windows-v2.php`, `case-studies-residential.php`, `generated-page.php` and `src/js/main.js`. This is a real merge with real conflicts, not a fast-forward. The owner is deciding who does that merge; the recommendation given was that whoever wrote `codex/tracking-repair` merges it into `main` first, then this session's work rebases on top and both ship together.
