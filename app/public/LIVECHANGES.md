...
- **The granular cookie-consent modal is live at scoped theme commit `23d31c4`.** A first visit opens a native modal that cannot be dismissed until the visitor chooses Customise or Accept all. Use necessary only remains inside Customise, where analytics and marketing are separate and off by default. Valid choices last 180 days, and footer Cookie settings supports withdrawal. Analytics gates Clarity and `FGV`/`FG2`; marketing gates Meta, advertising tags and persisted ad click IDs. Google Tag Manager receives category-specific Consent Mode signals before it loads. Legacy accepted/rejected strings are intentionally re-prompted.

## Current Truth

- Active GitHub repo: `https://github.com/0riceisnice0-hash/FensterGlazing-NewSite`
- **Live theme is at `23d31c4` on `codex/cookie-consent-live`, deployed 2026-07-30. Test is at `4bb8a48` on `main`, retaining the separate privacy-glass batch for review.** The pre-release checksum found that production contained the Ads production branch rather than the stale documented `572fe3c` pointer. The release was therefore rebuilt from theme-equivalent Ads release `814b8bf`; range `814b8bf..23d31c4` is five commits from one author and changes only the six cookie-consent theme files. Backup before it: `~/backups/fenster-theme/fenster-pre-23d31c4-20260730-145622.tar.gz` (375M, 1,739 entries, confirmed before deploying). Production verification: all six changed files byte-identical to the release candidate, six representative routes 200, mandatory Escape blocked, first layer exactly Customise and Accept all, necessary-only loaded no optional scripts, accept-all loaded Clarity/GTM/Meta, footer settings reopened, no console errors, and the 390x844 dialog fitted with zero horizontal overflow.
