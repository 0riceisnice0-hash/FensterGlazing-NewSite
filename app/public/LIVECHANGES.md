...
- **Live is at `5b7a612` on `main`, deployed 2026-07-29.** Live, `main` and test are all level. Backup before it: `~/backups/fenster-theme/fenster-pre-5b7a612-20260729-073434.tar.gz` (371M). The range `2f78837..5b7a612` was 11 commits, all one session's: the Leeds page rewritten off its scraped listicle, the casement page corrected to the U-value we can actually supply plus the glazing, acoustic and Part Q figures, and the Liniar foil range laid out on seven uPVC product routes via `template-parts/components/upvc-colour-grid.php`. The release before it was `2f78837` (footer), backup `fenster-pre-2f78837-20260729-062020.tar.gz`.
