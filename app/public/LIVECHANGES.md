...
- **Live is at `984e89c` on `main`, deployed 2026-07-27.** Live, `main` and test are all level. Backup before it: `~/backups/fenster-theme/fenster-pre-984e89c-20260727-083131.tar.gz` (364M). The range `4458fc6..984e89c` was 26 commits, all one session's: the product-selector hub rebuild (windows, doors and other services on one `product-hub.php`, replacing `windows-hub.php`), its imagery, the one-viewport tile grid, the decision panel and FAQs, and the Notan mark.
